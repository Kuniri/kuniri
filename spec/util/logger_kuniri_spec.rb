...
RSpec.describe Util::LoggerKuniri do
  ...
  context 'Single instance' do
    it 'Object instance should be the same' do
      object1 = Util::LoggerKuniri.create(STDOUT)
      object2 = Util::LoggerKuniri.create(STDOUT)
  ...
  context 'Update log level' do
    ...
    before :each do
      Util::LoggerKuniri.create(STDOUT)
      Util
    end

    it 'Default log level' do
      expect(Util::LoggerKuniri.create(STDOUT).level).to eq(Logger::WARN)
    end

    it 'Error log level' do
      Util::LoggerKuniri.update_log_level(Logger::ERROR)
      expect(Util::LoggerKuniri.create(STDOUT).level).to eq(Logger::ERROR)
    end

    it 'Info log level' do
      Util::LoggerKuniri.update_log_level(Logger::INFO)
      expect(Util::LoggerKuniri.create(STDOUT).level).to eq(Logger::INFO)
    end
    ...
    it 'Fatal log level' do
      ...
      expect(Util::LoggerKuniri.create(STDOUT).level).to eq(Logger::FATAL)
    end
